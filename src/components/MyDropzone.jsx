...
const UploadDangerAlert = ({onClick}) => (
    <div className="w-100 alert alert-danger" role="alert">
        ...
        <em>Please select an image to compare by clicking one of the thumbnails below or <span className="span-btn" onClick={onClick}>click here</span> to upload a new one instead:</em>
    </div>
);
                                ...
                                {({getRootProps, getInputProps, isDragActive}) => {
                                    return (
                                        <div {...getRootProps()} className={classNames('dropzone', {'dropzone--isActive': isDragActive})}>
                                            <input {...getInputProps()} />
                                            <DivStep title='Step 1' note={ isDragActive ? 'Drag and drop the source photo here...' : 'Drag and drop the source photo here or click to select a file to upload.'} /> 
                                        </div>
                                    )
                                }}
                                </Dropzone> : 
                            ...
                            { (imgStep2LoadStatus === 'not loaded') ? 
                                ...
                                {({getRootProps, getInputProps, isDragActive}) => {
                                    return (
                                        <div {...getRootProps()} className={classNames('dropzone', {'dropzone--isActive': isDragActive})}>
                                            <input {...getInputProps()} />
                                            <DivStep title='Step 2' note={ isDragActive ? 'Drag and drop the second photo here...' : 'Drag and drop the second source photo here or click to select a file to upload.'} /> 
                                        </div>
                                    )
                                }}
                                </Dropzone> : 
